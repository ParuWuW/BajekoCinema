...
public class SessionDAO {
	...
	// Revive old session update expiry and mark active
	...
	public void createSession(SessionModel session) throws SQLException {
		...
		try (Connection conn = DBconfig.getConnection();
				PreparedStatement ps = conn.prepareStatement(sql)) {
			ps.setString(1, session.getSession_id());
			ps.setInt(2, session.getUser_id());
			ps.setTimestamp(3, session.getExpires_at());
			ps.executeUpdate();
		}
	}
	...
	public SessionModel validateSession(String sessionId) throws SQLException {
		String sql = "SELECT * FROM Sessions WHERE session_id = ? AND is_active = 1 AND expires_at > NOW()";
		try (Connection conn = DBconfig.getConnection();
				PreparedStatement ps = conn.prepareStatement(sql)) {
			ps.setString(1, sessionId);
			ResultSet rs = ps.executeQuery();
